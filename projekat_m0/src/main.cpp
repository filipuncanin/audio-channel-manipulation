...
void signalProcessing(double pIn[][BLOCK_SIZE], double pOut[][BLOCK_SIZE], double* variableGains, int nSamples)
{
	for (int j = 0; j < nSamples; j++)
	{
		...
		pOut[L_CH][j] = pIn[L_CH][j] * variableGains[L_CH];
		pOut[R_CH][j] = pIn[R_CH][j] * variableGains[R_CH];
		if (outputMode > 0)	// outputMode == OUTPUT_MODE_2_2_0 || outputMode == OUTPUT_MODE_3_2_1
		{
			...
			pOut[LS_CH][j] = fabs(pOut[L_CH][j]);
			pOut[RS_CH][j] = fabs(pOut[R_CH][j]);
		}
		if (outputMode > 1)	// outputMode == OUTPUT_MODE_3_2_1
		{
			...
			pOut[LFE_CH][j] = saturation((pOut[LS_CH][j] + pOut[RS_CH][j]), limiterThreshold);
		}
	}
}
